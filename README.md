# Vitable Connect OpenAPI spec

This repository contains an [OpenAPI](https://spec.openapis.org/oas/v3.1.0.html) specification for the [Vitable Connect API](https://vitablehealth.com/support).

It is generated with [Stainless](https://www.stainless.com/).

## Spec

> [!CAUTION]
> This spec is primarily intended for Vitable Connect's internal use. Stability of the spec is not guaranteed.

[**View the spec**](openapi.yml)

## Clients

Our official clients are generated from this spec:

- [vitable-connect-typescript](https://github.com/Vitable-Inc/vitable-connect-typescript) (TypeScript)
- [vitable-connect-python](https://github.com/Vitable-Inc/vitable-connect-python) (Python)
- [vitable-connect-ruby](https://github.com/Vitable-Inc/vitable-connect-ruby) (Ruby)
